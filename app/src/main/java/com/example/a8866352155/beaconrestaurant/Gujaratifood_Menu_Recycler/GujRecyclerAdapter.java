...
public class GujRecyclerAdapter extends RecyclerView.Adapter<GujViewholder> {
    Context context;
    ArrayList<Guj> foods;
    Intent intent;

    public GujRecyclerAdapter(Context context, ArrayList<Guj> foods) {
        this.context = context;
        this.foods = foods;
    }

    @Override
    public GujViewholder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v= LayoutInflater.from(parent.getContext()).inflate(R.layout.all_acitivities_model,null);
        GujViewholder holder=new GujViewholder(v);
        return holder;    }

    @Override
    public void onBindViewHolder(GujViewholder holder, final int position) {
