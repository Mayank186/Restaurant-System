...
public class SouthFoodRecyclerAdapter extends RecyclerView.Adapter<SouthFoodViewholder> {
    Context context;
    ArrayList<SouthFood> foods;
    Intent intent;

    public SouthFoodRecyclerAdapter(Context context, ArrayList<SouthFood> foods) {
        this.context = context;
        this.foods = foods;
    }

    @Override
    public SouthFoodViewholder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v= LayoutInflater.from(parent.getContext()).inflate(R.layout.all_acitivities_model,null);
        SouthFoodViewholder holder=new SouthFoodViewholder(v);
        return holder;
    }

    @Override
    public void onBindViewHolder(SouthFoodViewholder holder, final int position) {
